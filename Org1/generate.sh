...
function generateChannelArtifacts() {
  ...
  echo
  echo "#################################################################"
  echo "### Generating channel configuration transaction 'public.tx' ###"
  echo "#################################################################"
  configtxgen -profile ThreeOrgsChannel -outputCreateChannelTx ./channel-artifacts/public.tx -channelID public
  if [ "$?" -ne 0 ]; then
  ...
  echo
  echo "#################################################################"
  echo "#######    Generating anchor peer update for Org1MSP - private1  ##########"
  echo "#################################################################"
  configtxgen -profile TwoOrgsChannel1 -outputAnchorPeersUpdate ./channel-artifacts/Org1MSPanchors.tx -channelID private1 -asOrg Org1MSP
  if [ "$?" -ne 0 ]; then
    echo "Failed to generate anchor peer update for Org1MSP..."
    exit 1
  fi
...
function replacePrivateKey () {
  ...
  CURRENT_DIR=$PWD
  cd crypto-config/peerOrganizations/org1/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
...
function networkUp () {
  ...
  if [ ! -d "crypto-config" ]; then
    ...
    replacePrivateKey
      ...
      echo "#################################################################"
      docker stack deploy --compose-file=docker-compose-kafka.yaml org1
  if [ $? -ne 0 ]; then
